...
import {
  Brain, Waves, AlertTriangle, TrendingUp,
  Activity, Settings, ShieldAlert, Info, Play, RotateCcw,
  Smartphone, Bell
} from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { riskClasses, proportionalPoints, blockageStates, trendReadings, preventiveStates } from '@/data/featureData';
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 border-b border-border/50 pb-3">
        ...
        <div>
          <h3 className="text-2xl font-bold text-foreground">1. ML Flood Prediction</h3>
          <p className="text-sm text-muted-foreground font-mono tracking-wide">SCIKIT-LEARN DECISION TREE</p>
        </div>
      </div>
      ...
      <Card className="border-2 border-border/50 overflow-hidden bg-card/60">
        <div className="grid grid-cols-1 md:grid-cols-12">
          ...
          <div className="md:col-span-5 p-6 border-r border-border/50 bg-muted/10 space-y-4">
            <p className="text-sm font-mono tracking-widest text-muted-foreground uppercase">Select Risk Input</p>
            <div className="flex flex-col gap-3">
              {riskClasses.map((rc) => (
                <button
                  ...
                  style={{
                    borderColor: activeRisk.level === rc.level ? rc.color : undefined,
                    background: activeRisk.level === rc.level ? rc.bgColor : undefined,
                  }}
                >
                  <div>
                    <p className="font-bold font-mono text-lg" style={{ color: rc.color }}>{rc.label}</p>
                    <p className="text-sm text-muted-foreground">Water: {rc.waterRange}</p>
                  </div>
                  {activeRisk.level === rc.level && <Play fill={rc.color} className="text-transparent" size={16} />}
                </button>
              ))}
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed mt-4 bg-background p-3 rounded-lg border border-border/50">
              <span className="font-bold text-foreground">Note:</span> Risk is predicted using a Decision Tree model running on Raspberry Pi 5.
            </p>
          </div>
               ...
               >
                 <div className="text-center space-y-3">
                   <div 
                     ...
                     style={{ color: activeRisk.color, borderColor: activeRisk.color, backgroundColor: activeRisk.bgColor }}
                   >
                     PREDICTION: {activeRisk.label}
                   </div>
                   <p className="text-lg text-muted-foreground">{activeRisk.description}</p>
                 </div>

                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                   <div className="rounded-2xl border bg-background p-5 shadow-sm text-center">
                     <p className="text-sm font-mono tracking-widest text-muted-foreground mb-3">COMMAND SENT</p>
                     ...
                     <p className="text-sm text-muted-foreground mt-2 inline-flex items-center gap-2">
                       <Settings size={14} className="text-fg-cyan" /> Gate Servo Angle
                     </p>
                   </div>
                   
                   <div className="rounded-2xl border bg-background p-5 shadow-sm">
                      <p className="text-sm font-mono tracking-widest text-muted-foreground mb-3 text-center">SYSTEM ACTIONS</p>
                      <ul className="space-y-2">
                        {activeRisk.actions.map(a => (
                          <li key={a} className="flex items-center gap-2 text-sm">
                            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: activeRisk.color }} />
                            <span>{a}</span>
                          </li>
                        ))}
                      </ul>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 border-b border-border/50 pb-3">
        ...
        <div>
          <h3 className="text-2xl font-bold text-foreground">2. Proportional Drain Control</h3>
          <p className="text-sm text-muted-foreground font-mono tracking-wide">DYNAMIC SERVO ANGLE</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="p-8 border-b md:border-b-0 md:border-r border-border/50 space-y-8 flex flex-col justify-center">
            ...
            <div>
              <div className="flex justify-between items-end mb-4">
                <span className="text-base font-bold text-foreground">Simulate Water Level</span>
                <span className="text-3xl font-mono text-fg-cyan font-bold">{sliderVal}%</span>
              </div>
            ...
            <div className="bg-muted/30 p-5 rounded-2xl border border-border/40 font-mono text-center">
               <div className="text-sm text-muted-foreground mb-2 tracking-widest uppercase">Equation</div>
               <div className="text-lg text-foreground">Angle = ({sliderVal}% ÷ 100) × 180°</div>
               <div className="text-2xl font-bold text-fg-cyan mt-2">= {calculatedAngle}°</div>
            </div>
          ...
          <div className="p-8 flex items-center justify-center relative min-h-[300px] bg-gradient-to-br from-background to-muted/20">
            <div className="relative w-64 h-64 border-4 border-border/60 rounded-full flex items-center justify-center overflow-hidden shadow-xl bg-card">
              ...
              {/* Liquid fill */}
              <div className="absolute bottom-0 inset-x-0 bg-fg-cyan/20 backdrop-blur-sm z-0" 
                   style={{ height: `${sliderVal}%`, transition: 'height 0.3s ease-out' }} />
              
              {/* Overlay text */}
              <div className="absolute top-8 font-mono text-sm tracking-widest text-muted-foreground font-bold z-20">GATE POSITION</div>
...
function BlockageFeature() {
  ...
  useEffect(() => {
    ...
    if (step === 1) { // 5s timer simulation (shortened for demo)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-border/50 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold text-foreground">3. Blockage Detection</h3>
            <p className="text-sm text-muted-foreground font-mono tracking-wide">AUTO-FLUSH LOGIC</p>
          </div>
        </div>
        <button onClick={() => setStep(0)} className="p-2 hover:bg-muted rounded-full transition-colors text-muted-foreground" title="Restart Demo">
          <RotateCcw size={18} />
        </button>
      </div>
      ...
      <Card className="border-2 border-border/50 bg-card overflow-hidden h-[360px] flex flex-col justify-center relative">
        <div className="absolute inset-0 pointer-events-none opacity-5" 
             style={{ background: `radial-gradient(circle at center, ${activeState.color} 0%, transparent 60%)` }} />
        ...
        <CardContent className="p-8 text-center space-y-6 relative z-10">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                {step === 3 ? (
                   <motion.div animate={{ rotate: [0, -45, 45, 0] }} transition={{ repeat: Infinity, duration: 0.8 }}>⚙️</motion.div>
                ) : activeState.icon}
              </div>
              ...
              <div>
                <p 
                  ...
                  style={{ color: activeState.color, borderColor: activeState.color, backgroundColor: `${activeState.color}10` }}
                >
                  {activeState.label}
                </p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 border-b border-border/50 pb-3">
        ...
        <div>
          <h3 className="text-xl font-bold text-foreground">4. Trend Preventive Action</h3>
          <p className="text-sm text-muted-foreground font-mono tracking-wide">EARLY PREDICTION A.I.</p>
        </div>
      </div>
      ...
      <Card className="border-2 border-border/50 bg-card overflow-hidden h-[360px] flex flex-col justify-between">
        ...
        <div className="p-6 pb-2">
          <div className="flex justify-between items-center mb-6">
            <span className="text-sm font-mono tracking-widest text-muted-foreground">LAST 10 READINGS</span>
            <span 
              className={cn("px-3 py-1 rounded-lg text-xs font-mono font-bold border transition-colors duration-500", 
                activeStage >= 1 ? "bg-fg-amber/20 text-fg-amber border-fg-amber/50" : "bg-muted text-muted-foreground border-border")}
            >
          ...
          <div className="h-32 relative w-full flex items-end gap-1.5 justify-between">
            ...
            {trendReadings.map((tr, i) => {
               ...
               return (
                 <div key={tr.index} className="flex-1 flex flex-col items-center gap-1 group relative">
                    {/* Tooltip */}
                    ...
                    {/* Rain dot */}
                    <div className="w-1.5 rounded-full bg-fg-cyan/60" style={{ height: activeStage === 0 ? '10px' : `${tr.rain/2}px`, marginBottom: '4px' }} />
                    {/* Water bar */}
                    <div 
        ...
        <div className="p-6 border-t border-border/50 bg-muted/10">
           <AnimatePresence mode="wait">
             ...
             >
               <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0 shadow-sm border border-border/50 bg-background"
                   style={{ color: st.color, boxShadow: `0 4px 12px ${st.color}20` }}>
                 {st.icon}
               </div>
               <div>
                 <h4 className="font-bold text-base" style={{ color: st.color }}>{st.label}</h4>
                 <p className="text-sm text-muted-foreground leading-snug">{st.desc}</p>
               </div>
             </motion.div>
           </AnimatePresence>
        </div>
